...
public class TickAcceleratorTileEntityTier2 extends TileEntity implements ITickableTileEntity {

    protected boolean isPoweredByRedstone;

    public TickAcceleratorTileEntityTier2() {
        super(ModTileEntityTypes.TICK_ACCELERATOR_BLOCK_TIER_2_TILE_ENTITY.get());
    }
    ...
    public void accelerateTick(World world, BlockPos pos) {
        ...
        if(!world.isClientSide && world instanceof ServerWorld){
            if (block.isRandomlyTicking(blockState) && world.getRandom().nextInt(30) == 0) {
        ...
        if (block.hasTileEntity(blockState)) {
            ...
            if (tileEntity != null && !tileEntity.isRemoved() && tileEntity instanceof ITickableTileEntity && checkIfNotTickAcceleratorBlock(tileEntity)) {
                for (int i = 0; i < TickAcceleratorConfig.TICK_ACCELERATOR_TIER_2_SPEED.get(); i++) {
